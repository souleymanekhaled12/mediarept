...
export default function NewArticle() {
  const [form, setForm] = useState({
    titre: '', resume: '', contenu: '', imageUrl: '', categorie: 'Actualité',
    auteur: 'Alassane Ibraima', publie: false,
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.imageUrl) { setError('Veuillez uploader une image.'); return; }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSuccess(`Article créé ! Slug : /${article.slug}`);
        setForm({ titre: '', resume: '', contenu: '', imageUrl: '', categorie: 'Actualité', auteur: 'Alassane Ibraima', publie: false });
      } else {
  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <div className="max-w-4xl mx-auto px-8 py-10">
        <h1 className="font-playfair text-3xl font-bold text-navy mb-8">Nouvel article</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white border border-border rounded-lg p-6 space-y-5">
            ...
            <div>
              ...
              <textarea value={form.resume} onChange={e => setForm(f => ({ ...f, resume: e.target.value }))} rows={2}
                ...
                placeholder="Bref résumé visible sur la page d'accueil..." />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select value={form.categorie} onChange={e => setForm(f => ({ ...f, categorie: e.target.value }))}
                  className="w-full border border-border rounded px-4 py-3 outline-none focus:border-navy transition-colors bg-white">
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-text2 mb-1.5">Image principale *</label>
              <input type="file" accept="image/jpeg,image/png,image/webp" onChange={uploadImage}
                className="block mb-3 text-sm text-text2" />
              {uploading && <p className="text-sm text-text3">Upload en cours...</p>}
              {form.imageUrl && (
                <div className="relative h-40 w-full rounded overflow-hidden border border-border">
            ...
            <div className="flex items-center gap-3">
              ...
              <label htmlFor="publie" className="text-sm font-semibold text-text2">Publier immédiatement</label>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            <button type="submit" disabled={saving || uploading}
              className="bg-rouge hover:bg-rouge2 text-white font-bold px-8 py-3 rounded transition-colors disabled:opacity-60">
              {saving ? 'Enregistrement...' : 'Créer l\'article'}
            </button>
            <Link href="/admin/articles"
              className="bg-bg border border-border hover:bg-border text-text2 font-semibold px-6 py-3 rounded transition-colors">
              Annuler
            </Link>
          </div>
        </form>
